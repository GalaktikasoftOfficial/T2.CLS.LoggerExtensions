...
{
	public class DurableHttpSender : TransportBase
	{
		...
		public DurableHttpSender(string[] requestUris,
			HttpClient client = null,
			string bufferPath = null,
			int? memoryBufferLimit = null,
			int? fileBufferLimit = null,
			int? senderBufferLimit = null,
			double? flushTimeout = null,
			int workerCount = 1,
			Encoding encoding = null,
			ILoggerFactory internalLoggerFactory = null)
		{
			var readLimit = Math.Min(Math.Max(senderBufferLimit ?? ReadLimitDefault, ReadLimitMin), ReadLimitMax);
		...
		protected virtual void HandleBuffer(IReadOnlyList<string> bufferBlock)
		{
			...
			using var stringContent = new StringContent(content); //, Encoding.Default, ContentType);
